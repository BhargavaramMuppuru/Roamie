 ...
 */
export function messageLooksLikeTripPlanningDetail(text: string): boolean {
  const t = text.trim();
  if (t.length < 2) {
    return false;
  }
  ...
  if (
    t.length <= 64 &&
    /^(thanks|thank you|thx|ty|ok+|k\.?|cool|great|nice|yep|yeah|yup|nope|nah|got it|gotcha|sounds good|sounds great|perfect|perfecto|awesome|love it|same|same here|👍|👎|❤️|🙏)/i.test(
      t,
    )
  ) {
    return false;
